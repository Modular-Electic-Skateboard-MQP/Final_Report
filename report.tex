...
The following figure outlines the general structure of the motor control system:

%\includesvg[width=\textwidth]{rt}
%\input{figs/rt.pdf_tex}
\includegraphics[width=\linewidth]{rt.pdf}
The remote control (RC) system is a standard 2.4 GHz hobbyist system designed for small unmanned RC cars and boats and comprised of a handheld transmitter, pictured below, and a small receiver which is attached to the vehicle being controlled. \\
\includegraphics[width=0.5\textwidth]{controller.jpg}
\\
The RC receiver outputs a pulse position modulation (PPM) signal that encodes the position of the throttle on the transmitter. This signal is read by the microcontroller (MCU). The MCU uses a PID control loop with throttle values, the temperatures of the primary and secondary batteries, and motor encoder readings as its inputs to determine the speed at which to drive the motors. The MCU software is structured as follows:\\
